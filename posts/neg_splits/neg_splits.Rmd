...
### The race(s)
This time I had to change the races used. The thing with the races in Santiago is that they must have uphill and downhill segments, which could affect the results. So I went for a flat race, and flat Berlin flat is. So the chosen race was the [Belrlin Halb Marathon](http://www.berliner-halbmarathon.de/en/).   
...
Most people runs positive splits, ranging from 0% to arround 8% slower in the second half.
...
From this charts its clear that theres is a relation between the speed difference and the total speed. Those with lower splits tend to run faster. But at the same time splits don't seem to explain the best times, fastest runners tend to have small splits, around cero.
...
ttstMf
```
We find the in this group the split sign still have a significat association with the speed, being positive splits slower (with a p-value of **`r sprintf("%0.3f", ttstMf$p.value)`**).
...
One notable thing is that having negative, or lower splits, have a stronger association with the overall speed than age. Meaning that much of the age disadvantage can be neutrilized with pace management.
